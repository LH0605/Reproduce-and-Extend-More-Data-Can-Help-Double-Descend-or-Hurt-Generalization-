...
def fit(num_epochs, x_train, y_train, x_test, y_test, model, opt, epsilon):
    model.train()
...
def main():
    ...
    epsilons = [0, 0.4, 0.6, 0.8, 1.1, 1.3, 1.5, 2.0, 2.5, 3.0, 5.0, 7.0, 9.0, 12.0, 15.0]
    ...
    for i in range(len(epsilons)):
        ...
        for train_size in range(1, TRAIN_SIZE+1):
            ...
            for j in range(N):
                ...
                test_loss = fit(num_epochs, x_train, y_train, x_test, y_test, model, opt, epsilon)
                temp[j] = test_loss
            mn = np.amin(temp)
            mx = np.amax(temp)
