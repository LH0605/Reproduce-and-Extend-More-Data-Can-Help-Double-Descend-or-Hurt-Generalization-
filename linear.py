...
def fit(num_epochs, train_loader, test_loader, model, loss_fn, opt, attack, train_size, epsilon):
    model.train()
    best_loss = float('inf')
...
def main():
    ...
    num_dim = args.ndim
    BEST_MODEL_PATH = f'best_gaussian_mixture_linear_loss_model_{args.attack}_{num_dim}D.pt'
    loss_fn = linear_loss
    ...
    for i in range(len(epsilons)):
        ...
        for train_size in range(1, TRAIN_SIZE+1):
            ...
            for j in range(N):
                ...
                test_loss = fit(num_epochs, train_loader, test_loader, model, loss_fn, opt, args.attack, train_size, epsilon)
